...
namespace Roslynator.CSharp.DiagnosticAnalyzers
{
    ...
    public class AddEmptyLineAfterBlockDiagnosticAnalyzer : BaseDiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics
        {
            get { return ImmutableArray.Create(DiagnosticDescriptors.AddEmptyLineAfterBlock); }
        }
        ...
        public override void Initialize(AnalysisContext context)
        {
            ...
            context.RegisterSyntaxNodeAction(f => AnalyzeCheckedStatement(f), SyntaxKind.CheckedStatement);
            context.RegisterSyntaxNodeAction(f => AnalyzeCheckedStatement(f), SyntaxKind.UncheckedStatement);
                    ...
                    if (nextStatement.GetSpanStartLine() - endLine == 1)
                    {
                        ...
                        if (trivia.IsEndOfLineTrivia())
                        {
                            context.ReportDiagnostic(
                                DiagnosticDescriptors.AddEmptyLineAfterBlock,
                                trivia.GetLocation());
                        }
                    }
                }
